...
Partial Class Change_Product_Group_List
    Inherits BaseClass
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            If Not IsPostBack Then
                ...
                oclsBind.BindDepartmentSelect(radDept, Val(Session("cmp_id")))
        ...
        Try
            For Each item As GridItem In rdcopyProduct.MasterTableView.Items
                ...
                If checkBox.Checked Then
                    ...
                    'If (radCategory.SelectedValue.ToString() IsNot "0" And radDept.SelectedValue.ToString() IsNot "0" And radCourse.SelectedValue.ToString() IsNot "0" And radUnit.SelectedValue.ToString() IsNot "0" And txtBase.Text.ToString() IsNot "") Then
                    oclsProduct.product_id = dataitem.GetDataKeyValue("product_id").ToString()
                    ...
                    oclsProduct.Unit_id = Val(radUnit.SelectedValue)
                    oclsProduct.Base = Val(txtBase.Text)
                    oclsProduct.cmp_id = Val(Session("cmp_id"))
                    oclsProduct.Ip_address = Request.UserHostAddress

                    If chkIsAvailable.Checked = True Then
                        oclsProduct.is_web_available = 1
                    Else
                        oclsProduct.is_web_available = 0
                    End If

                    If chkIsStock.Checked = True Then
                        oclsProduct.is_stock = 1
                    Else
                        oclsProduct.is_stock = 0
                    End If

                    If chkIsCondiment.Checked = True Then
                        oclsProduct.Is_Condiment = 1
                    Else
                        oclsProduct.Is_Condiment = 0
                    End If

                    If chkIsUnflagPrinter.Checked = True Then
                        oclsProduct.is_unflag_printer = 1
                    Else
                        oclsProduct.is_unflag_printer = 0
                    End If

                    oclsProduct.Change_ProductGroup()
                    'Else
                    '    ScriptManager.RegisterStartupScript(Me, Me.GetType, "test", "alert('Plese Select Category,Department,Unit,Course,Base');", True)
                    'End If
                End If
            ...
            Else
                ...
                chkIsCondiment.Checked = False
                chkIsStock.Checked = False
                chkIsUnflagPrinter.Checked = False
                txtBase.Text = ""
            End If
        Catch ex As Exception
