...
Partial Class Bank_Cash_Expense
    ...
    Dim oclsbnkexp As New Controller_clsExpense()
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            If Not IsPostBack Then
                ...
                oclsbnkexp.cmp_id = Val(Session("cmp_id"))
                oclsbnkexp.product_id = 0
                oclsbnkexp.SingleRecord = 0
                Dim dtSize As DataTable = oclsbnkexp.Select_SizeNPrice_By_Product()
    ...
    '    Try
    '        oclsbnkexp.cmp_id = Val(Session("cmp_id"))
    '        oclsbnkexp.form_name = "Stock Changes"
    '        Dim dtTable As DataTable = oclsbnkexp.Select1()
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        Try
            ...
            'If rdexpense.Visible = True Then
            ...
            For Each item As GridDataItem In rdexpense.Items
                For Each row As DataRow In dt1.Rows
                    If CType(item.FindControl("hdncashrow_Id"), HiddenField).Value.ToString() = row("row_Id").ToString() Then
                        row("cash_expense_amount") = Val(CType(item.FindControl("txtcashexpenseamount"), RadTextBox).Text)
                        row("cash_description") = (CType(item.FindControl("txtcashdescription"), RadTextBox).Text).ToString()
                    End If
                Next
            Next
        Catch ex As Exception
    ...
    Protected Sub Insert_Stk_detail()
        ...
        For Each row As DataRow In dt.Rows
            ...
            oclsbnkexp.cmp_id = Val(Session("cmp_id"))
            oclsbnkexp.Cash_Expense_Dcptn = row("cash_description")
            oclsbnkexp.Cash_Expense_Amount = row("cash_expense_amount")
            'oclsbnkexp.product_id = 0
            oclsbnkexp.Float_Value = row("cash_expense_amount")
            oclsbnkexp.Banking = row("cash_expense_amount")
            oclsbnkexp.ip_address = Request.UserHostAddress
            oclsbnkexp.login_id = Val(Session("login_id"))
            oclsbnkexp.expense_type = ddlexpense.SelectedValue
            oclsbnkexp.for_date = txtdate.SelectedDate
            oclsbnkexp.Location_id = rdlocation.SelectedValue

            'oclsbnkexp.stk_det_id = 0
            oclsbnkexp.Insert_Cash_Expense()
            Response.Redirect("Bank_Expense_List.aspx", False)
    ...
    Protected Sub Insert_Daily_Report()
        ...
        'oclsbnkexp.stk_det_id = 0
        oclsbnkexp.Insert_Daily_Report()
    ...
    Protected Sub rdproduct_ItemCommand(sender As Object, e As GridCommandEventArgs) Handles rdexpense.ItemCommand
        Try
            ...
            End If
        Catch ex As Exception
            LogHelper.Error("Bank_Cash_Expense:rdproduct_ItemCommand:" + ex.Message)
        End Try
    End Sub
