...
Partial Class BookingEasy_BookingDetailTable
    ...
    Public Sub FillSoreDeteils()
        ...
        lblDate.Text = Session("dtp_Date").ToString().Replace(" ", " at ")
        'Session("dtp_Date") = Nothing
        'BookingDateTime.Replace(" ", " at ")
    ...
    Private Sub SearchClient()
        ...
        Catch ex As Exception
            LogHelper.Error("bookingdetailtable:SearchClient" + ex.Message)
        End Try
    End Sub
    Private Sub GetAccountDetail(ByVal accountId As Integer)
        ...
        If (dr IsNot Nothing) Then
            ...
            Dim sql As String = "SELECT * FROM M_Custom_Field WHERE Enable_Val = 1 AND Static_Field = 'Custom' and  isnull(categoty_type,'B') = 'B' ORDER BY CAST(Sorting_no AS INT) ASC"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Request.QueryString("IsOnline") = "1" Then
            ...
            End If
            srchcli.Visible = False

        Else
            srchcli.Visible = True
        End If
    ...
    Protected Sub btnConfitmBooking_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnConfitmBooking.Click
        Dim oDbConnection As New DBConnection()
        If Request.QueryString("isonline").ToString() = 1 Then
            ...
            For Each gvItem As GridDataItem In gvServices.Items
                ...
                End If
            Next
            If Not chkcount.ToString() > 0 Then
                Dim selectproduct As String = "select is_selectProduct from BookingSchedule where BookingScheduleID = '" + BookingScheduleID.ToString() + "'"
                Dim is_selectProduct = oDbConnection.SingleCell(selectproduct)
                If is_selectProduct = 1 Then
                    Page.MaintainScrollPositionOnPostBack = True
                    ScriptManager.RegisterClientScriptBlock(Me, Me.[GetType](), "alertMessage", "alert('Kindly select at least one product/service for booking process.')", True)
                Else
                    Insert()
                End If
            Else
    ...
    Public Sub Insert()
        ...
        'Dim custEmail As Boolean = False
        'If CType(ucCustomer1.FindControl("df_Email1st"), TextBox).Text.ToString.Trim = "" Then
        '    custEmail = True
        'End If
        ...
        Else
            ...
            If objCommon.CheckDuplicate(em1, mo1, searchVal, Utils.ParseInt(hdnAccountID.Value)) And (hdnAccountID.Value.ToString() = "0" Or hdnAccountID.Value.ToString() = "") Then
            ...
            Else
                ...
                Dim bookingDate As DateTime = Convert.ToDateTime(BookingDateTime)

                Dim strBookingRef = New String(Enumerable.Repeat(chars, 8).[Select](Function(s) s(random.[Next](s.Length))).ToArray())

                strBookingRef = objCommon.CreateBookingInitiate(2, Utils.ParseInt(NoOfCovers), txtComment.Text, Utils.ParseInt(roomId), Utils.ParseDateTime(BookingDateTime), Nothing, 0, strBookingRef, 0, 0,
                                                        acctId, 1, TimeSpan.Parse(bookingDate.ToString("HH:mm")), Utils.ParseInt(drSetting("BookingSettingsid")), False, Sessions.UserID, Request.UserHostAddress, 0, BookingScheduleID, BookingScheduleDateId, Utils.ParseBool(IsOnline), StoreId)
                ...
                ' Insert Data Into "Tables" table for table Booking
                'Dim strFullName As String = ucCustomer1.FirstName & " " & ucCustomer1.LastName
                'Dim opentables As String = "Insert into tables (tableNumber, venueid, name, tablegroup, printed, training, Datetimeopened, covers, linkedaccountID, tablestate)"
                'opentables &= " VALUES ('" & strBookingRef & "'," & drStore("VenueID").ToString() & ",'" & strFullName & "','" & drSetting("TableGroup").ToString & "',0,0,'" & Utils.ParseDateTime(BookingDateTime).ToString("s") & "','" & NoOfCovers.ToString & "','" & acctId.ToString & "',1)"

                'conn.Ins_Upd_Del(opentables)


                Response.Redirect("Booking_Confirm.aspx?br=" + Utils.Encrypt(strBookingRef) + "&bs=" + Utils.Encrypt(BookingScheduleID()).ToString)

                ''Response.Redirect("TableBookConfime.aspx?BookingRef=" + strBookingRef)
    ...
    End Sub
    'Protected Sub txtSearchClient_TextChanged(sender As Object, e As System.EventArgs) Handles txtSearchClient.TextChanged
