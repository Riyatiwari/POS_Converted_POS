...
Partial Class BookingEasy_TableBookConfime_FirstPayment
    Inherits System.Web.UI.Page
    'Dim oClsDataccess As New ClsDataccess
    Public ReadOnly Property BookingRef() As String
        Get
            If Not String.IsNullOrEmpty(Request.Form("transactionUnique")) Then
                Return Request.Form("transactionUnique").ToString()
            Else
                Return String.Empty
            End If
        End Get
    ...
    Public ReadOnly Property Deposited() As String
        Get
            If Not String.IsNullOrEmpty(Request.Form("amountReceived")) Then
                Return Request.Form("amountReceived").ToString()
            Else
                Return String.Empty
            End If
        End Get
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            Dim refcode As String = Request.Form("responseCode").ToString
            If refcode = "0" Then
    ...
    Private Sub ConfirmBooking()
            ...
            Try
                ...
                If File.Exists(fileLoc) Then
                    Using sw As StreamWriter = New StreamWriter(fileLoc, True)
                        Dim order As String = Request.Form("orderRef")
                        If order = "" Then
                            order = "0"
                        End If
                        ...
                        'sw.WriteLine(DateTime.Now.ToString("MM/dd/yyyy") + "," + DateTime.Now.ToString("hh:mm tt") + ",," + Request.Form("transactionID").ToString + ",,,,,,,," + drBookingDetail("comment").ToString + ",,," + order.ToString() + ",,,,,," + drBookingDetail("bookingtotal").ToString + "," + Session("amountReceived").ToString() + "," + drBookingDetail("firstname").ToString + " " + drBookingDetail("Lastname").ToString + "," + Session("amountReceived").ToString() + ",," + drBookingDetail("accountid").ToString + "," + drBookingDetail("accnumber").ToString + "")
                        Dim DepositAmount As Decimal = Session("amountReceived").ToString()
        ...
        Else
            Dim strBookingID = oDbConnection.ExecuteNonQuery(" if not exists (select 1 from  bookings where bookingref=  '" + BookingRef() + "')	begin insert into bookings (covers, date, comment, Roomid, arrivaldate, departuredate, checkedin, bookingref, bookingtotal, deposit, accountid, period, bookingtime, BookingSettingsid, IsCanceled, CreatedDate, CreatedBy, IPAddress, GrandTotal, BookingScheduleID, BookingScheduleDateId, IsOnline, OurStoreId) select covers, date, comment, Roomid, arrivaldate, departuredate, checkedin, bookingref,bookingtotal, " + Session("amountReceived").ToString() + ", accountid, period, bookingtime, BookingSettingsid, IsCanceled, CreatedDate, CreatedBy, IPAddress, GrandTotal, BookingScheduleID, BookingScheduleDateId, IsOnline, OurStoreId from Booking_Initiate where bookingref= '" + BookingRef() + "' end")
            Dim drBookingDetail As DataRow = objCommon.GetBookingDetailByRef(BookingRef())
            Dim TranID = oDbConnection.ExecuteNonQuery("  if not exists (select 1 from  T_Payment_Transaction where Transaction_ref_no=  '" + Request.Form("transactionID").ToString + "')	begin INSERT INTO T_Payment_Transaction  ( Gateway_id, accountid, Booking_id, Transaction_ref_no, Amount, Currency, Trasaction_date, booking_type) VALUES  (0," + drBookingDetail("AccountId").ToString() + "," + drBookingDetail("bookingid").ToString() + ",'" + Request.Form("transactionID").ToString + "'," + Session("amountReceived").ToString() + ",'" + CurrencySymbol() + "',GETDATE(),0) end")
            Try
                Dim fileLoc As String = "c:\Temp\" + "Payment_Summary.txt"
                Dim fs As FileStream = Nothing
                If (Not File.Exists(fileLoc)) Then
    ...
    '    Label1.Text = Request.Form("responseCode")
    '    Label2.Text = Request.Form("amountReceived")
    '    Label3.Text = Request.Form("transactionID")
    '    Label4.Text = Request.Form("orderRef")
    'End Sub
End Class
