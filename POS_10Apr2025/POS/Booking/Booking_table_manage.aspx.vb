...
Partial Class BookingEasy_Booking_table_manage
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        If Sessions.TabID = 0 Or Sessions.TabID = Nothing Then
            lnkDashboard.PostBackUrl = "Dashboard.aspx?TabId=" + Utils.Encrypt("0") + ""
        Else
            lnkDashboard.PostBackUrl = "Dashboard.aspx?TabId=" + Utils.Encrypt(Sessions.TabID) + ""
        End If
        ...
        If Not IsPostBack Then
            Try
                ...
                If Sessions.UserID <> 0 Then
                    Dim UserID = Sessions.UserID
                    Sessions.UserID = UserID
                End If
                Dim conn As DBConnection = New DBConnection()
                Dim allStores As String
                allStores = "SELECT BS.BookingSettingsid,S.StoreName AS Name,BS.Color FROM bookingsettings BS left outer join StoreMaster S ON BS.StoreID = S.OurStoreId WHERE BS.IsActive = 1 AND (BS.bookingtype = '" & Utils.Encrypt("2") & "' OR BS.bookingtype = '" & Utils.Encrypt("3") & "')"

                If Session("ddlVenue") <> Nothing And Session("ddlType") <> Nothing Then
                    Dim dsTables As New DataSet
                    dsTables = conn.SelectData(allStores)

                    ddlVenue.DataSource = dsTables
                    ddlVenue.DataTextField = "Name"
                    ddlVenue.DataValueField = "BookingSettingsid"
                    ddlVenue.DataBind()
                    ddlVenue.SelectedValue = Session("ddlVenue")

                    Set_BackgroundColor()
                    BindType()
                    BindShape()
                    ddlType.SelectedValue = Session("ddlType")
                    binddiv()
                    bindcolor()
                Else
                    Dim dsTables As New DataSet
    ...
    Private Sub BindShape()
        ...
        radShape.DataBind()
        If Val(Session("radShapeVal")) <> 0 Then
            radShape.SelectedValue = Session("radShapeVal").ToString()
        Else
    ...
    Protected Sub Set_BackgroundColor()
        Try
            Dim conn As DBConnection = New DBConnection()
            Dim allStores As String = "select color from BookingSettings where BookingSettingsid =  " + ddlVenue.SelectedValue.ToString() + ""
            Dim dsTables As New DataSet
            dsTables = conn.SelectData(allStores)
    ...
    Private Sub BindType()
        ...
        If ddlVenue.Items.Count > 0 Then
            ...
            Dim ds As DataSet = obj.GetTableType(ddlVenue.SelectedValue, DateTime.Today)
    ...
    Private Sub binddiv()
        Dim oClsDataccess As New ClsDataccess
        Session("ddlVenue") = ddlVenue.SelectedValue
        Session("ddlType") = ddlType.SelectedValue
        ViewState("Script_text") = Nothing
        ViewState("Script_text_Click") = Nothing
        ViewState("Script_text_final") = Nothing
        ViewState("Script_offset") = Nothing
        ViewState("Script_text_O") = Nothing
        ViewState("Script_text_final_O") = Nothing
        ViewState("Script_offset_O") = Nothing
        rptDIV.DataSource = Nothing
        rptDIV.DataBind()
        rptOtherDIV.DataSource = Nothing
        rptOtherDIV.DataBind()
        Dim oColSqlPar As ColSqlparram = New ColSqlparram()
        oColSqlPar.Add("@BookingScheduleID", ddlType.SelectedValue)
    ...
    Protected Sub radShape_SelectedIndexChanged(sender As Object, e As Telerik.Web.UI.DropDownListEventArgs) Handles radShape.SelectedIndexChanged
        ...
        rpShapeVal.DataSource = dsTable
        rpShapeVal.DataBind()
        Session("radShapeVal") = radShape.SelectedValue
    End Sub
        ...
        Try
            ...
            If ddlSize.SelectedIndex <> -1 Then
                ...
                If a = 0 Then
                    ...
                    Dim oColSqlparram As ColSqlparram = New ColSqlparram()
                    oColSqlparram.Add("@BookingScheduleID", Val(ddlType.SelectedValue), SqlDbType.Int)
                    ...
                    If Val(Session("EditShape")) <> 0 Then
                        oColSqlparram.Add("@Tran_id", hdnShapeId.Value.ToString().Split("#")(0).Replace("Slb", ""), SqlDbType.Int)
        ...
        Try
            ...
            If Val(Session("openPopup2")) = 1 Then
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    lblBalance.Text = ds.Tables(0).Rows(0)("Balance").ToString() + " " + CurrencySymbol().ToString()
    ...
    Protected Sub lnkExport_Click(sender As Object, e As System.EventArgs)
        ...
        Response.AddHeader("Content-Disposition", "attachment; filename=" + ddlType.SelectedText.ToString() + ".png")
    ...
    Protected Sub lnkSaveBackground_Click(sender As Object, e As System.EventArgs)
        Try
            ...
            Dim str As String = "update BookingSettings set color = '" + color.ToString() + "' where BookingSettingsid = " + ddlVenue.SelectedValue.ToString() + " "
    ...
    '<WebMethod()> _
    'Public Function ColorChange(ColorVal As String) As String
    '    Try
    '        Dim inColor As Color = Color.FromName(ColorVal)
    '        Dim InAmount As Double = 0.2
    '        Dim RetColor As Color = Color.FromArgb(inColor.A, CInt(Math.Min(255, inColor.R + 255 * InAmount)), CInt(Math.Min(255, inColor.G + 255 * InAmount)), CInt(Math.Min(255, inColor.B + 255 * InAmount)))
    ...
    'Public Property ColorChange1(ColorVal As String) As String
    '    Get
    '            Dim inColor As Color = Color.FromName(ColorVal)
    '            Dim InAmount As Double = 0.2
    '            Dim RetColor As Color = Color.FromArgb(inColor.A, CInt(Math.Min(255, inColor.R + 255 * InAmount)), CInt(Math.Min(255, inColor.G + 255 * InAmount)), CInt(Math.Min(255, inColor.B + 255 * InAmount)))
