...
Partial Class BookingEasy_TableBookConfime
    ...
    Public ReadOnly Property Deposited() As String
        Get
            If Not String.IsNullOrEmpty(Request.Form("x_amount")) Then
                Return Request.Form("x_amount").ToString()
            Else
                Return String.Empty
            End If
        End Get
                ...
                Else
                    ...
                    If field1 = "0" Then
                        ...
                        Else
                            aHome.HRef = "~/Booking/Dashboard.aspx?TabId=" + Utils.Encrypt(Sessions.TabID) + ""
                        End If
                        'aHome.HRef = "~/Booking/Dashboard.aspx"
                    ElseIf field1 = "1" Then
                        If Session("WidType") = "Table" Then
                            aHome.HRef = "~/BookingEasy/TableBookingWidget.aspx"
                        Else
    ...
    Private Sub ConfirmBooking()
        ...
        Dim dtAccount As DataSet = oDbConnection.SelectData(" select 1 from Booking_Initiate where Roomid <> 0 and bookingref = '" + BookingRef() + "'")
        If dtAccount.Tables(0).Rows.Count = 0 Then
            Dim strBookingID = oDbConnection.ExecuteNonQuery(" if not exists (select 1 from  bookings where bookingref=  '" + BookingRef() + "')	begin insert into bookings (covers, date, comment, Roomid, arrivaldate, departuredate, checkedin, bookingref, bookingtotal, deposit, accountid, period, bookingtime, BookingSettingsid, IsCanceled, CreatedDate, CreatedBy, IPAddress, GrandTotal, BookingScheduleID, BookingScheduleDateId, IsOnline, OurStoreId) select covers, date, comment, Roomid, arrivaldate, departuredate, checkedin, bookingref, bookingtotal, " + Request.Form("x_amount").ToString() + ", accountid, period, bookingtime, BookingSettingsid, IsCanceled, CreatedDate, CreatedBy, IPAddress, GrandTotal, BookingScheduleID, BookingScheduleDateId, IsOnline, OurStoreId from Booking_Initiate where bookingref= '" + BookingRef() + "' end")
            Dim drBookingDetail As DataRow = objCommon.GetBookingDetailByRef(BookingRef())
            Dim TranID = oDbConnection.ExecuteNonQuery("  if not exists (select 1 from  T_Payment_Transaction where Transaction_ref_no=  '" + Request.Form("x_invoice_num").ToString + "')	begin INSERT INTO T_Payment_Transaction  ( Gateway_id, accountid, Booking_id, Transaction_ref_no, Amount, Currency, Trasaction_date, booking_type) VALUES  (0," + drBookingDetail("AccountId").ToString() + "," + drBookingDetail("bookingid").ToString() + ",'" + Request.Form("x_invoice_num").ToString + "'," + Request.Form("x_amount").ToString() + ",'" + CurrencySymbol() + "',GETDATE(),1) end")
            Try
                ...
                If File.Exists(fileLoc) Then
                    Using sw As StreamWriter = New StreamWriter(fileLoc, True)
                        ...
                        Dim DepositAmount As Decimal = Request.Form("x_amount").ToString()
            ...
            'Dim string_qry As String = "select 1 from bookings b Inner join BookingSchedule s on s.BookingScheduleID = b.BookingScheduleID where b.bookingid = " + booking_id.ToString() + " and (CONVERT(VARCHAR(10),GETDATE(),120) between CONVERT(VARCHAR(10),S.StartDate,120) and CONVERT(VARCHAR(10),S.EndDate,120)) and (CONVERT(VARCHAR(15),GETDATE(),108) between CONVERT(VARCHAR(15),S.StartTime,108) and CONVERT(VARCHAR(15),S.EndTime,108))"
        ...
        Else
            Dim strBookingID = oDbConnection.ExecuteNonQuery(" if not exists (select 1 from  bookings where bookingref=  '" + BookingRef() + "')	begin insert into bookings (covers, date, comment, Roomid, arrivaldate, departuredate, checkedin, bookingref, bookingtotal, deposit, accountid, period, bookingtime, BookingSettingsid, IsCanceled, CreatedDate, CreatedBy, IPAddress, GrandTotal, BookingScheduleID, BookingScheduleDateId, IsOnline, OurStoreId) select covers, date, comment, Roomid, arrivaldate, departuredate, checkedin, bookingref, bookingtotal, " + Request.Form("x_amount").ToString() + ", accountid, period, bookingtime, BookingSettingsid, IsCanceled, CreatedDate, CreatedBy, IPAddress, GrandTotal, BookingScheduleID, BookingScheduleDateId, IsOnline, OurStoreId from Booking_Initiate where bookingref= '" + BookingRef() + "' end")
            Dim drBookingDetail As DataRow = objCommon.GetBookingDetailByRef(BookingRef())
            Dim TranID = oDbConnection.ExecuteNonQuery("  if not exists (select 1 from  T_Payment_Transaction where Transaction_ref_no=  '" + Request.Form("x_invoice_num").ToString + "')	begin INSERT INTO T_Payment_Transaction  ( Gateway_id, accountid, Booking_id, Transaction_ref_no, Amount, Currency, Trasaction_date, booking_type) VALUES  (0," + drBookingDetail("AccountId").ToString() + "," + drBookingDetail("bookingid").ToString() + ",'" + Request.Form("x_invoice_num").ToString + "'," + Request.Form("x_amount").ToString() + ",'" + CurrencySymbol() + "',GETDATE(),0) end")
            Try
                Dim fileLoc As String = "c:\Temp\" + "Payment_Summary.txt"
                Dim fs As FileStream = Nothing
                If (Not File.Exists(fileLoc)) Then
    ...
    '    ViewState("Description") = ViewState("Description").ToString + "," + CType(e.Item.FindControl("hfName"), HiddenField).Value.ToString
    'End Sub
End Class
