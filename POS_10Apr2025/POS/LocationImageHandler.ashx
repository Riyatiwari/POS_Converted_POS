<%@ WebHandler Language="VB" Class="LocationImageHandler" %>
...
Public Class LocationImageHandler : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        Try
            ...
            If dt.Rows.Count > 0 Then
                ...
                If Val(context.Request.QueryString("location_id").ToString()) > 0 Then

                    oColSqlPar.Add("@location_id", context.Request.QueryString("location_id"))
                    oColSqlPar.Add("@imageno", context.Request.QueryString("imageno"))
                    LogHelper.Error("POS_WS:ProcessRequest:3")
                    ' imgdt = GetdataTableSp("WS_Get_till_Image", oColSqlPar, "WS_Get_till_Image", getconstr(dt))
                    imgdt = GetdataTableSp("WS_Get_Location_Image", oColSqlPar, "WS_Get_Location_Image", getconstr(dt))
                    LogHelper.Error("POS_WS:ProcessRequest:4" + imgdt.Rows.Count.ToString() + getconstr(dt).ToString())
                End If

                ' Dim oColSqlPar As ColSqlparram = New ColSqlparram()
                ' oColSqlPar.Add("@location_id", context.Request.QueryString("lid"))

                ' Dim imgdt As DataTable = GetdataTableSp("Get_Location_Image", oColSqlPar, "Get_Location_Image", getconstr(dt))
                If imgdt.Rows.Count > 0 Then
                    LogHelper.Error("POS_WS:ProcessRequest:5")
                    context.Response.BinaryWrite(DirectCast(imgdt.Rows(0)("limage"), Byte()))
