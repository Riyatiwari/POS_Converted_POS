...
namespace Converted_POS.Pages
{
    public class Receipt : PageModel
    {
        ...
        public Receipt(IConfiguration configuration)
        {
            try
            {
        ...
        public void OnGet(string store, string table_uuid)
        {
            try
            {
                //string store = Request.Query["store"];
               ...
               // CheckStore(Request.Query["store"].ToString());
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("Get_credit_sales_for_email", connection))
                    {
                            ...
                            if (dtS.Rows.Count > 0)
                            {
                                for (int sales = 0; sales <= 0; sales++)
                                {
                                    if (Convert.ToDouble(dtS.Rows[sales]["total_amount"].ToString()) > 0)
                                    {
                                        ...
                                        builder.Append("<div id='printr' style='width:auto; color:#000000; font-family:verdana;border:1px solid  #000000;'>");
                                       ...
                                       // builder.Append("</td></tr>");
                                        ...
                                        {
                                            using (SqlCommand cmd = new SqlCommand("Get_tsales_for_email", connection))
                                            {
